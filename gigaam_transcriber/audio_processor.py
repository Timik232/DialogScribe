"""
Модуль обработки аудио для транскрипции.

Обеспечивает:
- Конвертацию аудио в оптимальный формат (16kHz, mono, PCM)
...
class AudioProcessor:
    """Обработка аудио для транскрипции."""

    # Параметры аудио для транскрипции
    ...
    def _get_duration_fallback(self, path: Path) -> float:
        ...
        try:
            ...
            raise AudioProcessingError(
                f"Не удалось определить длительность файла", file_path=str(path), cause=e
            )

    def get_media_info(self, path: Path | str) -> dict[str, Any]:
        """
    ...
    def normalize(
        ...
        """
        Нормализация аудио в формат, оптимальный для транскрипции.
        ...
        Args:
            ...
            denoise: Уровень шумоподавления: "none", "light" (anlmdn), "medium" (afftdn)
    ...
    def extract_audio_from_video(
        ...
        Args:
            ...
            normalize: Нормализовать аудио для транскрипции (16kHz, mono)
    ...
    def prepare_for_gigaam(
        ...
        """
        Подготовка файла для транскрипции.

        Автоматически определяет тип файла (audio/video) и
        конвертирует в формат, оптимальный для транскрипции.
    ...
    def split_by_silence(
        ...
        try:
            ...
            detect_nonsilent = importlib.import_module("pydub.silence").detect_nonsilent
        except ImportError:
    ...
    def split_audio(
        ...
        remainder = total_duration - (num_chunks * chunk_duration)
    ...
    def load_audio_segment(
        ...
        try:
            ...
            raise AudioProcessingError("soundfile не установлен. Установите: pip install soundfile")
